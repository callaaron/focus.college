/**
 * Seed Assessment Questions to Cloudflare D1 Database
 * 
 * This script generates SQL INSERT statements for 49 assessment questions
 * covering 8 dimensions and 35 core competencies.
 * 
 * Usage:
 * 1. Run this script to generate SQL: tsx scripts/seed-questions-d1.ts > /tmp/seed.sql
 * 2. Execute SQL with wrangler: npx wrangler d1 execute focus-college-db --remote --file=/tmp/seed.sql
 */
...
function generateSql() {
  console.log("-- 评估系统题库数据");
  console.log("-- 共49道题目，覆盖8大维度35项核心能力");
  console.log("-- 生成时间:", new Date().toISOString());
  console.log("");
  
  // First, clear existing questions
  console.log("-- 清空现有题目");
  console.log("DELETE FROM assessmentQuestions;");
  console.log("");
  
  // Generate INSERT statements
  console.log("-- 插入49道评估题目");
  assessmentQuestions.forEach((q, index) => {
    const sql = `INSERT INTO assessmentQuestions (
  ...
  console.log("-- 验证数据");
  console.log("SELECT COUNT(*) as total FROM assessmentQuestions;");
  console.log("SELECT competencyId, COUNT(*) as count FROM assessmentQuestions GROUP BY competencyId ORDER BY competencyId;");
}
