...
export default function Competencies() {
  ...
  if (competenciesLoading || comparisonLoading) {
    return <PageSkeleton />;
  }
    ...
    <DashboardLayout>
      <PageTransition>
        <div className="space-y-6">
          ...
          <FadeInUp>
            <div>
              <h1 className="text-3xl font-bold tracking-tight">能力看板</h1>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{notStartedCount}</div>
              <p className="text-xs text-muted-foreground mt-1">
                发展空间
              </p>
            </CardContent>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {Math.round(radarData.reduce((sum, d) => sum + d.您的得分, 0) / radarData.length)}
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">我的平均分</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-slate-600">
                    {Math.round(radarData.reduce((sum, d) => sum + d.行业平均, 0) / radarData.length)}
                  </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-1">
                              <div className="flex justify-between text-xs text-muted-foreground">
                                <span>掌握度</span>
                                <span>{(comp.userProgress.currentLevel || 0) * 20}%</span>
                              </div>
                              <Progress 
