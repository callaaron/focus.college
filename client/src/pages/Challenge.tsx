...
export default function Challenge() {
  ...
  const isLoading = challengeLoading || statsLoading;
  const isAlreadyCompleted = dailyChallenge?.isCompleted || showResult;

  return (
    <DashboardLayout>
      <PageContainer isLoading={isLoading} pageTitle="今日挑战" pageDescription="每天完成管理能力小挑战，持续提升您的实战水平">
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 *:data-[slot=card]:shadow-xs">
            <Card className="@container/card bg-gradient-to-t from-amber-50/50 to-card dark:from-amber-950/30">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">连续打卡</p>
              ...
              <CardFooter className="flex-col items-start gap-1 pt-0 text-xs">
                <div className="line-clamp-1 flex gap-1.5 font-medium text-amber-600 dark:text-amber-400">
                  继续保持 <Flame className="size-3.5" />
                </div>
                <div className="text-muted-foreground">养成习惯</div>
              </CardFooter>
            </Card>
            ...
            <Card className="@container/card bg-gradient-to-t from-emerald-50/50 to-card dark:from-emerald-950/30">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">累计完成</p>
              ...
              <CardFooter className="flex-col items-start gap-1 pt-0 text-xs">
                <div className="line-clamp-1 flex gap-1.5 font-medium text-emerald-600 dark:text-emerald-400">
                  已完成 <CheckCircle2 className="size-3.5" />
                </div>
                <div className="text-muted-foreground">挑战记录</div>
              </CardFooter>
            </Card>
            ...
            <Card className="@container/card bg-gradient-to-t from-primary/5 to-card dark:from-primary/10">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">获得积分</p>
              ...
              <CardFooter className="flex-col items-start gap-1 pt-0 text-xs">
                <div className="line-clamp-1 flex gap-1.5 font-medium text-yellow-600 dark:text-yellow-400">
                  可兑换 <Star className="size-3.5" />
                </div>
            ...
            <Card className="@container/card bg-gradient-to-t from-purple-50/50 to-card dark:from-purple-950/30">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">排名</p>
              ...
              <CardFooter className="flex-col items-start gap-1 pt-0 text-xs">
                <div className="line-clamp-1 flex gap-1.5 font-medium text-purple-600 dark:text-purple-400">
                  全站排名 <Trophy className="size-3.5" />
                </div>
                <div className="text-muted-foreground">{stats?.rank ? '继续攀升' : '暂无排名'}</div>
              </CardFooter>
            </Card>
          </div>
                    ...
                    {leaderboard.map((item: any, index: number) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className="text-lg font-bold text-yellow-600 tabular-nums">{item.totalPoints}</p>
