...
export default function AssessmentResults() {
  ...
  if (isLoading) {
    return (
      <DashboardLayout>
        <PageContainer isLoading pageTitle="评估报告" pageDescription="正在加载您的评估结果..." />
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout>
      <PageContainer pageTitle="评估报告" pageDescription="恭喜完成评估！以下是您的能力评估结果">
        <div className="max-w-6xl mx-auto space-y-5">
          ...
          <Card className="@container/card bg-gradient-to-t from-primary/5 to-card dark:from-primary/10">
            ...
            <CardContent className="text-center space-y-4">
              <div className={`text-6xl font-bold tabular-nums ${getScoreColor(data.overallScore)}`}>
                {data.overallScore}
              </div>
                ...
                {data.categoryResults.map((category) => (
                  <div key={category.category} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`font-bold tabular-nums ${getScoreColor(category.avgScore)}`}>
                        {category.avgScore} 分
                      </span>
                    </div>
                ...
                {data.competencyResults.map((comp) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      <span className={`text-sm font-bold tabular-nums ${getScoreColor(comp.avgScore)}`}>
                        {comp.avgScore}
                      </span>
                    </div>
